...
import { getShifts } from 'apiSdk/shifts';
...
function DashboardPage() {
  ...
  const shiftStat = useSWR<PaginatedInterface<ShiftInterface>>(
    () => `/shifts`,
    () => getShifts({ limit: 1, offset: 0 }),
  );

  const applicationStat = useSWR<PaginatedInterface<ApplicationInterface>>(
    () => `/applications`,
    () => getApplications({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Shifts',
      result: shiftStat?.data?.totalCount,
      isLoading: shiftStat.isLoading,
    },
    {
      title: 'Applications',
      result: applicationStat?.data?.totalCount,
      isLoading: applicationStat.isLoading,
    },
  ];
