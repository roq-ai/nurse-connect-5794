...
import { getShiftById, updateShiftById } from 'apiSdk/shifts';
...
function ShiftEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ShiftInterface>(
    () => (id ? `/shifts/${id}` : null),
    () => getShiftById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ShiftInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateShiftById(id, values);
      mutate(updated);
      resetForm();
      router.push('/shifts');
    } catch (error) {
  ...
  const formik = useFormik<ShiftInterface>({
    initialValues: data,
