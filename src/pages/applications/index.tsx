...
type ColumnType = ColumnDef<ApplicationInterface, unknown>;
...
export function ApplicationListPage(props: ApplicationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ApplicationInterface>>(
    () => `/applications?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ApplicationInterface) => {
    if (hasAccess('application', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/applications/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'status', header: 'Status', accessorKey: 'status' },
    hasAccess('shift', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'shift',
          header: 'Shift',
          accessorKey: 'shift',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/shifts/view/${record.shift?.id}`}>
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/applications/view/${record.id}`} passHref legacyBehavior>
                <Button
